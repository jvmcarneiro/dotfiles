...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey          }, "Escape", function () awful.spawn("powermenu") end,
              {description = "power-menu prompt", group = "launcher"}),
    awful.key({ modkey, "Shift" }, "Return",
        function ()
            local float_terms = function (c)
                return awful.rules.match(c, {pid = float_term_pid})
            end
            if float_term_pid == "" then
                float_term_pid = awful.spawn(terminal .. " --class floating-term")
            else
                for c in awful.client.iterate(float_terms) do
                    if c:isvisible() then
                        c.minimized = true
                    else
                        c.minimized = false
              ...
              {description = "floating terminal", group = "launcher"}),
...
awful.rules.rules = {
    ...
    -- Opens floating terminal in the center and always on top
...
client.connect_signal("manage", function (c)
    ...
    -- Spawn floating clients centered and round corner
    if c.floating then
        c.placement = awful.placement.centered 
        c.shape = function(cr,w,h)
            gears.shape.rounded_rect(cr,w,h,5)
        end
    elseif awful.layout.layouts == awful.layout.suit.floating then
        c.placement = awful.placement.centered
        c.shape = function(cr,w,h)
            gears.shape.rounded_rect(cr,w,h,5)
        end
