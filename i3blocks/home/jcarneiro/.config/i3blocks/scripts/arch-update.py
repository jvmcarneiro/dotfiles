...
def create_argparse():
    ...
    parser.add_argument(
        ...
        '--aur',
        action='store_true',
        help='Include AUR packages. Attn: Trizen must be installed'
    )
    parser.add_argument(
        '-q',
        '--quiet',
...
def get_aur_updates():
    output = ''
    try:
        output = check_output(['trizen', '-Qua']).decode('utf-8')
    except subprocess.CalledProcessError as exc:
        # trizen exits with 1 and no output if no updates are available.
